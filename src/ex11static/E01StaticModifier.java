...
/*
 ...
 -클래스 외부
 	...
 	2. 인스천스 생성없이 클래스명으로 직접 접근한다. (권장사항)
...
class MyStatic{
	/*
	 ...
	 참조변수를 통해 접근해야한다. 즉, 일반적인 멤머변수는 인스턴스를
	 생성하는 시점에 메모리에 로드된다.
	...
	/*
	 정적변수: 인스턴스를 생성할 필요없아 클래스명으로 바로 접근할 수 
	 있다, JVM(자바가상머신)이 프로그램을 시작할 떄 미리 메모리(Method 영역)에 
	 로드된다.
	 */
	...
	/*
	 정적멤버메서드: 정적메서드 내부에서는 인스턴스형 멤버에는 접근할 수 
	 	없다. 정적 멤버만 접근 가능하다.
	 	(※main메서드에서는 Static으로 선언된 메서드만 호출할 수 있었던것을 
	 	기억하자)
	 */
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		 ...
		 */
		//이와 같이 작성하면 누가 보더라도 같은 변수임을 한 눈에 알 수 있다.
