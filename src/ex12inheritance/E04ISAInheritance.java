...
/*
 ...
 따라서 실제 업무에서도 유사한 기능의 클래스들을 상속으로 묶어서 정의 하는것이
 좋다.
 */
// 컴퓨터: 기본적인 컴퓨팅 환경을 제공하여 입력된 내용을 계산한다.
class Computer{
	String owner; //멤버변수: 소유주
	
	//디폴트 생성자 public computer()	{}
	/*
	 매개변수가 있는 인자 생성자 추가. 이 경우 디폴트 생성자는 추가되지 않는다.
	 */
	...
	/*
	 ...
	 상속받은 클래스에 접근이 불가능하므, public으로 선언된 
...
class NotebookComputer extends Computer{
	...
	public void movingCal() {
		...
		/*
		 ...
		 자식클래에서도 접근할 수 없어 에러가 발생한다. 이 경우
...
/*
 테블릿: 컴퓨터의 기본계산기능과, 휴대이용 그리고 전용펜으로
...
class TableNotebook extends NotebookComputer{
	...
	public void write(String penInfo) {
		//배터리가 충분한지 확인
		if(battery<1) {
			System.out.println("배터리가 방전되어 사용블가");
			return;
		}
