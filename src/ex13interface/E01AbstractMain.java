...
/*
 추상클래스
 - 상속을 목적으로 제작되므로 인슽너스 생성은 허용하지 않는다.
 - 추상 클래스는 대부분 추상메서드를 포함하고 있으므로, 이를 상속받은 클래스는
 반그시 추상메서드를 오버라이딩 해야한다.
...
//추상클래스1: 추상엠서를 포함하지 않은 클래스
abstract class AbsClass1{
	//int형으로 표현할 수 있는 최대값을 정적상수로 선언
	public static final int MAX_INT = Integer.MAX_VALUE;
	
	
	void instanceMethod() {} //일반적인 멤버메서드
	static void staticMethod() {} //정적메서드
}
/*
 추상메소드를 포함하지 않은 추상클래스를 상속하면 오버라이딩은 강제사항이
...
/*
 위 클래스를 상속하면 부모가 가진 추상메서드를 그대호 상속받게 되므로
...
class AbsClassChild2 extends AbsClass2{
	/*
	 오버라이딩은 부모쪽의 메서드를 가리고, 새로운 기능을 재정의 하는 것
	 이므로 해당 클래스는 추상메서드를 포함하지 않은 상태가 된다. 
	 따라서 인스턴스 생성이 가능해진다.
