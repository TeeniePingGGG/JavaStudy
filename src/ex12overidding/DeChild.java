...
public class DeChild extends DeParent {
	...
	public void study() {
		/*
		 부모릐 멤버변수 name은 private이므로 자식에서
	...
	/*
	 ...
	 하였으므로 자식쪽에서는 보이지 않아 오보라이딩의 대상이
	 될수 없다. 이 메서드는 자식쪽에서 확장한 메서드가 된다.
	 */
	private void eat() {
	...
	/*
	 메서드 오버라이딩의 조건
	 - 부모클래스에 선언될떄와 비교해서 접근범위가 동일하거나 
	 그보다 넓으면 가능하다.
	 - 부모에서 default로 선언하면 잣식에서는 default 혹은
	...
	/*
	 메서드의 형태가 부모와는 다르므로 오버라이딩이 될수 없다.
