...
class MyChild extends MyParent{
	...
	/*
	 메서드명은 동일하짐나 매개변수가 틀리므로 오버로딩으로 정의한 메서드.
	 따라서 자식에서 확장한 멤버가 된다.
	 */
...
public class E06Polymorphism {
	...
	public static void main(String[] args) {
		...
		MyChild myChild = new MyChild(); //동질화: 참조변수와 인스턴스의 타입이 동일
		...
		/*
		 이질화: 부모타입의 변수로 자식 인스턴스를 참조라는것을 말한다.
...
//		parent1.childMember = 1;
		...
		//부모 참조변수로 자식 인스턴스를 참조한 두번쨰 인스턴스 생성
...
//		(MyChild)myParent.childMember=1;
		...
		/*
		 자식타입으로 형변환 이후에 해당 참조볌수를 통해 자식 멤버에
		 접근한다. 위와 동일한 방식이다.
		 */
