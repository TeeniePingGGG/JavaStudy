...
public class E04OverrideApply {
	...
	public static void main(String[] args) {
		 ...
		 */
		System.err.println("Child형 참조변수 Child객채 참조");
		
		//자식인스턴스를 자식 참조변수를 통해 참조한다.(동일한 타입)
		DeChild child = new DeChild("율곡이이", 49, "00학번");
		
		/*
		 오버라이딩 처리된 메서드이므로 참조변수와 상관없이 항상
		 자식쪽 메서드가 호출된다. 메서드 오버라이딩은 자식쪽에 정의한
		 메서드사 부모쪽에 정의된 메서드를 가리게된다. 따라서 오버라이딩된
		 메서드를 호출할떄는 참조변수의 영향을 받지 않는다.
		 */
		child.exercise();
		child.sleep();
		child.printParent();
		//자식쪽에서 확장한 메서드는 부모에는 정의되어 있지 않다.
		child.study();
