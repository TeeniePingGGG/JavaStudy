...
/*
 ...
 - 객체간에 형변환이 되려면 반드시 두 클래스 간에 상속관계가 있어여한다.
 - 부모타입의 참조변수로 자식 인스턴스를 참조할수 있으므로 매개변수를
...
public class E05InstanceOf {
	...
	/*
	 ...
	 한꺼번에 받을 수 있다.
	 - Box 타입으로 인수를 받으므로 자동형변환(업캐스팅)되어 자식 클래스의
	 멤버메서드를 호출할 수 없게 된다.
	 - 이떄 자식의 멤버메서드를 호출하시 위해 강제형변환(다운캐스팅)이 필요하다
	 - instaceof 연산자를 통해 자식타입으로 형변환이 가능한지 판단한 후
