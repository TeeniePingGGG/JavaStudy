...
/*
 ...
 - Array하는 이름처럼 '배열'의 특성을 사지고 있어 index를 통해
 저장 및 접근이 가능하다.
 */
public class Ex03ArrayList1 {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 ArratList와 나머지 컬렉션들은 데이터를 저장하는 내부적인
...
//		list.add(5,"소방차"); 에러발생됨
		...
		/*
		 중복저장: List는 배열의 특성을 가지므로 데이터를 중복
		 	저장할 수 있다. 동일한 데이터라도 index로 구분할수 있기
		 	때문이다. add()는 추가에 성공할 경우 true를
		 	반환한다.*/
		...
		/*
		 출력하기
		 방법1: 일반 for문 사용. 이떄 인덱스를 통해 접근해야
		 	하므로 get() 메서드를 사용한다.
		 */
		System.out.println("\n[일반 for문 사용]");
		...
		System.out.println(list.contains("블랙핑크")? "블랙핑크잇음":"블랙핑크 없음");
		
		/*
		 삭제하기
		 방법1: 인덱스로 삭제
		 	인덱스를 통해 삭제한 후 완료되먄 해당 인스턴스의
		...
		/*
		 방법 2: 방법1과 동일하게 인덱스로 삭제한다. 단 indexOf러
		 	인스턴스의 인덱스를 찾은 후 이 값을 사용한다.
		 */
		...
		/*
		 방법3: 인스턴스의 참조값을 통해 삭제한다. 이 경우에는
		 	삭제에 성공할 경우 true가 반환된다. 즉 boolean타입의
		 	반환값을 갖는다.
		 */
		//삭제 성공
		...
		list.removeAll(list);
		list.clear();
		System.out.println("전체삭제후 객체 수:"+list.size());
	}
