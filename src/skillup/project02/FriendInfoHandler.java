...
public class FriendInfoHandler {
	...
	private int numOfFriends; //정보 카운트용 변수 생성
	
	
	//생성자
	public FriendInfoHandler(int num) {
		/*
		 핸들러 클래스의 인스턴스를 생성할떄 데이터 저장을 위한 Friend
		 타입의 배열의 인스턴스도 함께 생성한다. 크기는 num으로 지정한다.
		 */
		myFriends = new Friend[num]; 
		numOfFriends=0; //카운트용 변수는 0으로 초기화
	}
	...
	public void addFriend(int choice) {
		...
		/*
		 High, Univ 타입의 인스턴스를 Friend타입의 배열에 추가할때에는
		 ...
		 public void deleteInfo() {
			// Scanner scan = new Scanner(System.in);
			 ...
			 if(deleteIndex==-1) {
				 ...
				 System.out.println("### 데이터("+deleteIndex+"번)"
				 		+ "가 삭제되었습니다 ###");
			 }
		 }
}
