...
/*
 ...
 :메인쓰레드와 Working쓰레드(개발자다 정의한 쓰레드)를 일턷는 것으로
 메인쓰레드가 종료되어도 계속 실행되며. Dead상태가 되어야만 실행이
 종료되는 쓰레드를 말한다. 해당 쓰레드는 main() 메서드가 종료된 이후도
 계속 실행되는것을 볼 수 있다.
 */
class NormalThread extends Thread{
	...
	/*
	 ...
	 직접 처리하지 않고, throws해서 호출한 위치로 예외 던지기를 하고
	 있다.
	 */
	void threadMethod() throws InterruptedException{
		for(int i=1; i<=10; i++) {
			...
			/*
			 10번 반복 중 2초씩 block상태로 돌아가므로 총 20초간 
			 반복된다.
			 */
			sleep(2000);
		}
...
/*
 ...
 프로그램이 종료될떄 함께 종료되므로 무한루프로 구현한다.
 */
...
public class Ex03DaemonThread {
	...
	public static void main(String[] args) {
		...
		nt1.clasMethod();
		new NormalClass("두번쨰 클래스").clasMethod();
		
		/*
		 첫번쨰 독립스레드 생성 및 시작. 우선순위를 별도로 부여하지 않으므로
		 5로 지정된다,스레드 인스턴스 생성시 이름을 붜여하지 않는 방법으로
		 아래 2가지가 있다. */
		
		//방법1: 생성자를 통해 스레드의 이름부여
		...
		/*
		 두번쨰 독립스레드 생성 및 시작. 쓰레드명을 부여하지 않았으므로
		 JVM이 자동으로 이름을 부여한다.
		 */
		...
		System.out.println("현재활성호 된 상태의 쓰레드 수"+
				Thread.activeCount());
